"""
Case-Based Assessment Generator service.
Generates case-based assessment bundles based on course materials.
"""
...
from app.services.assessment.prompts import CASE_BASED_GENERATION_PROMPT
...
async def generate_case_based(
    ...
    Args:
        ...
        num_questions: Number of cases to generate (default: 1, max: 50)
    ...
    Returns:
        List of dictionaries, each with a case-based assessment bundle
    """
    try:
        # Build query for retrieval
        query = "Generate case-based assessments with realistic scenarios about key concepts in this course material"
        ...
        if difficulty:
            ...
            if difficulty_lower == "easy":
                difficulty_instruction = "- Difficulty: EASY - Focus on basic case scenarios with straightforward questions\n"
            elif difficulty_lower == "medium":
                difficulty_instruction = "- Difficulty: MEDIUM - Require application of concepts to case scenarios\n"
            elif difficulty_lower == "hard":
                difficulty_instruction = "- Difficulty: HARD - Require analysis and synthesis of complex case scenarios\n"

        # Log for observability
        docs_count = len(set(s.get("document_id", "") for s in sources if s.get("document_id")))
        logger.info(
            f"Generating case_based assessment: course_id={course_id}, "
        ...
        for i in range(num_questions):
            prompt = CASE_BASED_GENERATION_PROMPT.format(
                context_str=context,
            ...
            try:
                response_text = await asyncio.wait_for(
                    ...
                    timeout=45.0  # Longer timeout for more complex generation
                )
            except asyncio.TimeoutError:
                logger.error("Case-based generation timed out after 45 seconds")
                raise RuntimeError("Case-based generation timed out. Please try again.")

            # Parse JSON response
            try:
                case_data = _parse_json_response(response_text)
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse case_based JSON: {e}")
                logger.error(f"Response text: {response_text[:500]}")
                raise ValueError("Failed to generate valid case-based assessment. Please try again.")
            ...
            # Add type field
            case_data["type"] = "case_based"

            # Merge case-level sources with question-level sources
            case_sources = _extract_source_references(sources)
            case_data["sources"] = case_sources
            
            # Ensure each question has sources (use case sources if missing)
            # Map "source_references" from AI response to "sources" for consistency
            if "questions" in case_data:
                for question in case_data["questions"]:
                    # If AI returned "source_references", rename it to "sources"
                    if "source_references" in question and "sources" not in question:
                        question["sources"] = question.pop("source_references")
                    # If no sources at all, use case-level sources
                    if "sources" not in question or not question["sources"]:
                        question["sources"] = case_sources
