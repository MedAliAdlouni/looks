...
def _generate_content_sync(prompt: str) -> str:
    ...
    for model_name in fallback_models:
        ...
        for variant in model_variants:
            ...
            try:
                ...
                logger.info(f"Using fallback model {variant} for content generation")
                if not response.text:
                    raise ValueError(f"Empty response from model {variant}")
                return response.text
            except (ClientError, ValueError) as e:
                ...
                try:
                    ...
                    logger.info(f"Using fallback model {variant} (without JSON mode) for content generation")
                    if not response.text:
                        raise ValueError(f"Empty response from model {variant}")
                    return response.text
                except (ClientError, ValueError) as e2:
    ...
    if last_error:
        logger.error(f"All Gemini models failed for content generation. Last error: {last_error}")
        raise last_error
    else:
        raise RuntimeError("All Gemini models failed for content generation and no error was captured")
